...
functions over I2C. It is very similar to the 6 DoF MPU6050 for which an
extensive library has already been built.
Most of the function of the MPU9150 can utilize the MPU6050 library.

This Arduino sketch utilizes Jeff Rowberg's MPU6050 library to generate the
basic sensor data
for use in two sensor fusion algorithms becoming increasingly popular with DIY
quadcopter and robotics engineers.
I have added and slightly modified Jeff's library here.

This simple sketch will demo the following:
* How to create a MPU6050 object, using a constructor (global variables
section).
...
* How to display output at a rate different from the sensor data update and
fusion filter update rates
* How to specify the accelerometer and gyro sampling and bandwidth rates
* How to use the data from the MPU9150 to fuse the sensor data into a quaternion
representation of the sensor frame
  orientation relative to a fixed Earth frame providing absolute orientation
information for subsequent use.
* An example of how to use the quaternion data to generate standard aircraft
orientation data in the form of
  Tait-Bryan angles representing the sensor yaw, pitch, and roll angles suitable
for any vehicle stablization control application.
...
digital pins 5 - 9 described below. If using SPI you might need to press one of
the A0 - A3 pins
...
// System)
#define PI 3.141592
#define GyroMeasError                                                          \
  PI *(40.0f /                                                                 \
       180.0f) // gyroscope measurement error in rads/s (shown as 3 deg/s)
#define GyroMeasDrift                                                          \
  PI *(                                                                        \
      0.0f /                                                                   \
      180.0f) // gyroscope measurement drift in rad/s/s (shown as 0.0 deg/s/s)
...
#define zeta                                                                   \
  sqrt(3.0f / 4.0f) * GyroMeasDrift // compute zeta, the other free parameter in
                                    // the Madgwick scheme usually set to a
                                    // small or zero value
#define Kp                                                                     \
...
void setup() {
  ...
  // in the table below.
  // The accelerometer output rate is fixed at 1kHz. This means that for a
  // Sample
...
void loop() {
  if (mpu.getIntDataReadyStatus() ==
    ...
    //  reading is +1 g in the positive direction for each axis.
    //  The gyro should read zero for each axis when the sensor is at rest.
    //  Small or zero adjustment should be needed for these sensors.
    //  The magnetometer is a different thing. Most magnetometers will be
    //  sensitive to circuit currents, computers, and
    //  other both man-made and natural sources of magnetic field. The rough way
    //  to calibrate the magnetometer is to record
    //  the maximum and minimum readings (generally achieved at the North
    //  magnetic direction). The average of the sum divided by two
    ...
    if (mcount > 1000 / MagRate) { // this is a poor man's way of setting the
      ...
      my = m2 * 10.0f * 1229.0f / 4096.0f + 70.0f; // apply calibration offsets
  ...
  deltat = subTimespec(&now, &lastUpdate) / 1000000000.0f;
  // update
  lastUpdate = now;
  ...
  // For the MPU-9150, we have chosen a magnetic rotation that keeps the sensor
  ...
  printf(" ay = ");
  printf("%d",(int)(1000 * ay));
  printf(" az = ");
  printf("%d",(int)(1000 * az));
  printf(" mg\n");
  printf("gx = ");
  printf("%F", gx);
  printf(" gy = ");
  ...
  */
  ...
  // Tait-Bryan angles as well as Euler angles are non-commutative; that is, the
  // get the correct orientation the rotations must be
  // applied in the correct order which for this configuration is yaw, pitch,
  // and then roll.
  // For more see
...
// device orientation -- which can be converted to yaw, pitch, and roll. Useful
